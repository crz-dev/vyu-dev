...
var CircularSampleBuffer = class {
  ...
  ensureCapacity(minCapacityFrames) {
    ...
    for (let frame = 0; frame < this._frameCount; frame += 1) {
      const sourceIndex =
        ((this._readFrame + frame) % this._capacityFrames) *
        SAMPLES_PER_FRAME$1;
  ...
  pushSamples(source, sourceFrameOffset = 0, frameCount = 0) {
    const sourceStartSample =
      Math.max(0, Math.floor(sourceFrameOffset)) * SAMPLES_PER_FRAME$1;
    const availableFrames = Math.max(
      0,
      Math.floor((source.length - sourceStartSample) / SAMPLES_PER_FRAME$1),
    );
    ...
    for (let frame = 0; frame < framesToWrite; frame += 1) {
      ...
      const destIndex =
        ((writeFrame + frame) % this._capacityFrames) * SAMPLES_PER_FRAME$1;
  ...
  readSample(sampleIndex) {
    ...
    const sourceIndex =
      ((this._readFrame + frameOffset) % this._capacityFrames) *
        SAMPLES_PER_FRAME$1 +
      channelOffset;
  ...
  dropFrames(frameCount) {
    const framesToDrop = Math.max(
      0,
      Math.min(Math.max(0, Math.floor(frameCount)), this._frameCount),
    );
...
function resolveKernelRegistration(
  ...
  return resolveKernelRegistration(
    requireRegisteredStrategy(registration.baseStrategy),
    visited,
  );
}
function normalizeParams(registration, params) {
  const defaults = registration.defaultParams;
  if (registration.normalizeParams !== void 0)
...
var RateTransposer = class RateTransposer extends AbstractSamplePipe {
  ...
  setInterpolationStrategy(strategy) {
    ...
    if (this.applyKernelParams !== void 0)
  ...
  setInterpolationStrategyParams(params) {
    ...
    for (const [key, value] of Object.entries(params))
      if (value !== void 0) nextParams[key] = value;
...
var CircularStretchInputBufferAdapter = class {
  ...
  readSubarray(start, end) {
    ...
    const requestedSamples =
      Math.max(normalizedStart, Math.floor(end)) - normalizedStart;
 ...
 */
var createCircularStretchInputBufferAdapter = () =>
...
var SoundTouchProcessor = class extends SoundTouchProcessorBase {
  ...
  constructor(options) {
    super("[SoundTouchProcessor]", {
      sampleRate,
      sampleBufferType:
        options?.processorOptions?.sampleBufferType ?? "circular",
