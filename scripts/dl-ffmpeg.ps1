...
$tarOk = $false
try {
  tar -xf $archive -C $tempDir --wildcards "*/bin/ffmpeg.exe" "*/bin/ffprobe.exe" 2>$null
...
if (-not $tarOk) {
  ...
  if (-not $innerDir) {
    ...
    if ($sevenZip) {
      & $sevenZip.Source x $archive -o"$tempDir" -y "*/bin/ffmpeg.exe" "*/bin/ffprobe.exe" | Out-Null
    } else {
...
Write-Host "  ffmpeg.exe: $((Get-Item (Join-Path $extractTo "ffmpeg.exe")).Length / 1KB KB) KB"
Write-Host "  ffprobe.exe: $((Get-Item (Join-Path $extractTo "ffprobe.exe")).Length / 1KB KB) KB"
